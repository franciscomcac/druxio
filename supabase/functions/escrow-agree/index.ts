...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Verify caller is the seller (expert on the accepted quote)
    ...
    if (!quote || quote.expert_id !== userId) {
      return new Response(JSON.stringify({ error: "Only the seller can agree to the transaction" }), {
        status: 403,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    // Call Escrow.com API to agree as the seller (using As-Customer header)
    ...
    if (!patchRes.ok) {
      console.error("Escrow agree API error:", JSON.stringify(patchData));
    ...
    await serviceClient
      .from("jobs")
      .update({ escrow_status: "awaiting_funding" })
      .eq("id", jobId);

    // Notify buyer that seller agreed, they need to fund
    ...
    if (jobFull) {
      await serviceClient.from("notifications").insert({
        ...
        type: "escrow_agreed",
        title: "Seller agreed to the transaction",
        message: "The seller has agreed. Please fund the escrow to proceed.",
        data: { job_id: jobId, escrow_txn_id: job.escrow_txn_id },
      });
    }
    ...
    console.error("Escrow agree error:", err);
