...
const categories = [
  { name: "Tech", slug: "tech", icon: <Code className="h-6 w-6" />, subcategories: ["Discord Bots", "Web Dev", "SEO", "Server Setup", "App Dev", "WordPress"], color: "from-primary/15 to-primary/5" },
  ...
  { name: "Music", slug: "music", icon: <Music className="h-6 w-6" />, subcategories: ["Production", "Mixing", "Guitar", "Piano", "Vocals", "Beat Making"], color: "from-primary/12 to-primary/5" },
  ...
  return (
    <section id="categories" className="relative bg-background py-16 sm:py-28 overflow-hidden">
      ...
      <div className="container relative z-10 mx-auto px-4">
        {/* Right-aligned header on desktop */}
        <div className="mb-10 sm:mb-16 flex flex-col items-start md:items-end gap-3">
          ...
          <p className="text-muted-foreground animate-fade-in [animation-delay:200ms]">
            There's a verified expert ready to help, right now.
          </p>
        </div>

        {/* Featured first card (2x2), rest normal */}
        <div className="grid gap-3 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
          {categories.map((cat, i) => {
            ...
            return (
              <div
                key={cat.name}
                className={`group rounded-sm border border-border bg-card transition-all duration-300 hover:border-primary/30 hover:shadow-glow animate-slide-up cursor-pointer ${
                  isFeatured
                    ? "sm:col-span-2 sm:row-span-2 p-6 sm:p-8 flex flex-col justify-between"
                    : "flex sm:flex-col items-center sm:items-start gap-4 sm:gap-0 px-4 py-4 sm:p-6"
                }`}
                style={{ animationDelay: `${i * 60}ms` }}
                onClick={() => navigate(`/category/${cat.slug}`)}
              >
                <div className={`shrink-0 flex items-center justify-center rounded-xl sm:rounded-2xl bg-gradient-to-br ${cat.color} text-primary transition-transform duration-300 group-hover:scale-110 ${
                  isFeatured ? "h-14 w-14 sm:h-16 sm:w-16 mb-6" : "h-11 w-11 sm:h-12 sm:w-12 sm:mb-5"
                }`}>
                  {cat.icon}
                </div>
                <div className="flex-1 min-w-0">
                  <h3 className={`font-semibold text-foreground ${isFeatured ? "text-xl sm:text-2xl mb-2 sm:mb-4" : "text-base sm:text-lg sm:mb-3"}`}>{cat.name}</h3>
                  {isFeatured ? (
