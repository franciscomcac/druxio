    ...
    <section id="how-it-works" ref={ref} className="bg-background py-12 md:py-24">
      <div className="container mx-auto px-4">
        <div className="mb-8 md:mb-14 max-w-lg reveal">
          <p className="mb-2 md:mb-3 text-xs md:text-sm font-semibold uppercase tracking-widest text-primary">How It Works</p>
          <h2 className="mb-3 md:mb-4 text-2xl md:text-4xl font-bold text-foreground leading-tight">
        ...
        <div className="grid gap-3 md:gap-4 grid-cols-2 lg:grid-cols-4 mb-8 md:mb-12">
          {steps.map((step, i) => (
            <div
              key={i}
              className={`group relative rounded-lg border border-border bg-card p-4 md:p-6 transition-all duration-300 hover:border-primary/40 hover:bg-card/80 reveal delay-${i * 100 + 100}`}
            >
              <span className="text-2xl md:text-4xl font-bold text-foreground font-mono block mb-2 md:mb-4">
                {step.num}
              </span>
              <div className="mb-3 md:mb-5 flex h-8 w-8 md:h-10 md:w-10 items-center justify-center rounded-lg bg-primary/10 text-primary">
                {step.icon}
              </div>
              <h3 className="mb-1 md:mb-2 text-sm md:text-lg font-semibold text-foreground">{step.title}</h3>
