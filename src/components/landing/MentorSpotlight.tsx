...
const MentorSpotlight = () => {
  return (
    <section className="relative bg-background py-28 overflow-hidden">
      <div className="absolute top-[20%] right-[5%] h-[400px] w-[400px] rounded-full bg-primary/[0.03] blur-[120px] pointer-events-none" />

      <div className="container relative z-10 mx-auto px-4">
        <div className="mb-16 flex flex-col items-start gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="mb-3 text-sm font-semibold uppercase tracking-widest text-primary animate-fade-in">Live Feed</p>
            <h2 className="mb-3 text-4xl font-bold text-foreground animate-fade-in [animation-delay:100ms]">Recent Requests</h2>
            <p className="text-muted-foreground animate-fade-in [animation-delay:200ms]">See what people are getting help with right now</p>
          </div>
          <Link to="/post-request" className="animate-fade-in [animation-delay:300ms]">
            <Button className="gap-2 shadow-glow hover:shadow-glow-lg transition-shadow duration-500">Post Your Request <ArrowRight className="h-4 w-4" /></Button>
          </Link>
        </div>
